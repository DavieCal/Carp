...
dotchart(Delta$LFCCR, xlab = "Fish Count",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$DF,  ylab = "Dead Fish Count")
dotchart(Delta$DF, xlab = "Dead Fish Count",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Pelicans,  ylab = "Pelicans Count")
dotchart(Delta$Pelicans, xlab = "Pelicans Count",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Cormorant,  ylab = "Cormorant Count")
dotchart(Delta$Cormorant, xlab = "Cormorant Count",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Grebes,  ylab = "Grebe Count")
dotchart(Delta$Grebes, xlab = "Grebe Count",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Exposure,  ylab = "Exposure")
dotchart(Delta$Exposure, xlab = "Exposure",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Image_Clarity,  ylab = "Image_Clarity")
dotchart(Delta$Image_Clarity, xlab = "Image_Clarity",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Water_Clarity,  ylab = "Water_Clarity")
dotchart(Delta$Water_Clarity, xlab = "Water_Clarity",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Discharge_cms,  ylab = "Discharge_cms")
dotchart(Delta$Discharge_cms, xlab = "Discharge_cms",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Velocity_m_s,  ylab = "Velocity_m_s")
dotchart(Delta$Velocity_m_s, xlab = "Velocity_m_s",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$Argo_Depth_m,  ylab = "Argo_Depth_m")
dotchart(Delta$Argo_Depth_m, xlab = "Argo_Depth_m",
         ylab = "Order of the data")

par(mfrow= c (1,2), mar = c(5,4,2,1))
boxplot(Delta$DucksUnlimited_Temp,  ylab = "DucksUnlimited_Temp")
dotchart(Delta$DucksUnlimited_Temp, xlab = "DucksUnlimited_Temp",
         ylab = "Order of the data")
...
#Figure 3
library(lattice)
Z <- cbind(Delta$LFCCR, Delta$DF,  Delta$Pelicans,
           Delta$Grebes,  Delta$Cormorant, Delta$Discharge_cms)

colnames(Z) <- c("LFCCR", "DF", "Pelicans","Grebes", "Cormorant", "Discharge_cms")
...
# the [] brackets can be used to limit the anaylsis to a subset
# in this case it was limited to three months of the data with the code below
#Sparrows$I1 <- Sparrows$fMonth =="June" |
#  Sparrows$fMonth =="July" |
#  Sparrows$fMonth =="August"
...
Delta$fMonth<-factor(Sparrows$Month,
                        levels = c(5, 6, 7, 8, 9, 10),
                        labels = c("May", "June", "July", "August",
                                   "Sept.", "Oct."))
...
# breaks adjusts how many bins are set up in the histogram
#x11() opens new window
x11()
hist(Delta$LFCCR[Delta$LFCCR],
     xlab = "LFCCR", breaks = 30,
     main = "", ylab = "Frequency")
...
#These are all the species
AllS <- c(
  "LFCCR",     "DF",     "Pelicans",     "Grebes",     "Cormorant",
  "Percent_Obstructed",     "Discharge_cms",     "DucksUnlimited_Temp"
  )
...
#################################################################################
#Table 1
Delta2 <- read.table(file = "DeltaNoZero.txt", header = TRUE)


names(Delta2)
#[1] "Order"               "Image_Name"          "SiteNo"              "SideNo"              "LFC"                 "LFCC"               
#[7] "LFCCR"               "DF"                  "Pelicans"            "Grebes"              "Cormorant"           "Percent_Obstructed" 
#[13] "Exposure"            "Image_Clarity"       "Water_Clarity"       "Month"               "Day"                 "Year"               
#[19] "Time"                "Discharge_cms"       "Velocity_m_s"        "Argo_Depth_m"        "Logger_Depth_m"      "Logger_Temp_C"      
#[25] "DucksUnlimited_Temp"

#Load our own library files
source("HighstatLib.R")

#Seclect covraites
Z<-Delta2[,c("DF", "Pelicans", "Water_Clarity",
                "Discharge_cms", "Velocity_m_s", "Argo_Depth_m", "DucksUnlimited_Temp")]

corvif(Z)      #Part of Table 1

#Run linear regression
M1<-lm(LFCCR~ DF + Pelicans + Water_Clarity + Discharge_cms + Velocity_m_s +Argo_Depth_m+ DucksUnlimited_Temp, data = Delta2)
summary(M1)    #Part of Table 1


#Chop out covariates
Z<-Delta2[,c("Pelicans", "Water_Clarity",
             "Discharge_cms", "Argo_Depth_m","DucksUnlimited_Temp")]

corvif(Z)      #Part of Table 1

#Run linear regression
M1<-lm(LFCCR~  Pelicans + Water_Clarity + Discharge_cms + Argo_Depth_m + DucksUnlimited_Temp, data = Delta2)
summary(M1)    #Part of Table 1
...
xyplot(Y10 ~ Z | ID11, col = 1,
       ...
       scales = list(alternating = T,
                     ...
                     y = list(relation = "same")),
       xlab = "Covariates",
       #cex is the font size of the par.strip.text
       par.strip.text = list(cex = 0.8),
       ylab = "Banded",
       panel=function(x, y, subscripts,...){
         panel.grid(h =- 1, v = 2)
...
Z <- as.vector(as.matrix(Delta2[, c("DF", "Pelicans",  
                                    "Discharge_cms", "Velocity_m_s", "Argo_Depth_m", "DucksUnlimited_Temp"
)]))


#Setup the data in vector format for the xyplot, number in blue is the number of covariates
Y10 <- rep(Delta2$LFCCR, 6)

MyNames <- names(Delta2[,c("DF", "Pelicans",
                           "Discharge_cms", "Velocity_m_s", "Argo_Depth_m", "DucksUnlimited_Temp")])

ID10 <- rep(MyNames, each = length(Delta2$LFCCR))
library(lattice)


ID11 <- factor(ID10, labels = c("DF", "Pelicans", 
                                "Discharge_cms", "Velocity_m_s", "Argo_Depth_m", "DucksUnlimited_Temp"),
               levels = c("DF", "Pelicans", 
                          "Discharge_cms", "Velocity_m_s", "Argo_Depth_m", "DucksUnlimited_Temp"))
...
plot(Time, Delta$LFCCR, type = "l", xlab = "15 minute interval",
     ylab = "LFCCR")
acf(Delta$LFCCR, main = "15 minute interval ACF")
