...
where several variables are the same as in the Poisson model with the additon of $Exposure_i$ for camerea exposure level of the picture, $Water Clarity_i$ for water clarity level in the picture, and $Image Clairty_i$ for the image clarity level of each picture.
...
The temperature and temperature^2^ covariates are correlated as are hour and hour^2^, let's centre these variables and re-test for collinearity.
...
where $Y_i$ is a $\theta$ is the overdispersion parameter such that $var(Y) = \mu + \frac{\mu^{2}}{\theta}$ and $\mu_i$ is a function of the envrionmental variables:
